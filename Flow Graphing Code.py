...
An instrument is underperforming if any of the following conditions occur: 
    - Flow rate dips to more than 4 mL/min lower than the set flow rate AND less than 40% of the set flow rate
    - Flow rate dips for more than 800ms
    - Median total flow rate is not within 2.5% of the set flow rate

Note: The file Blaze.py must be in the same file as this one for this script to work.
"""
...
# Colours for the graph. Only the first two should be used, but if something unexpected occurs, up to six can be used
COLORS = (['b', 'g', 'r', 'm', 'c', 'peru'])

# Initialize arrays to store files and data for the blaze runs
path = sys.path[0]
files = []
file_names = []
blazes = []
...
# Method opens all .txt files in the directory and stores them
def openFiles():
...
# Method to close all files used
def closeFiles():
    for f in files:
        f.close()


# Method takes the Blaze objects used and graphs all of them.
# Data acquired from the same run (indicated by the same instrument, ratio and TFR) will be plotted on the same graph
def plot():
    for run_index in range(0, len(blaze_runs)):
        ...
        # Variables to help determine whether a run passed or failed
        expected = 0
        median = 0
        resolutions = []
        dip_magnitudes = []
        ...
        for j in range(0, len(blazes)):
            # Plot and get information from a Blaze run if it matches the run currently being graphed
            if blazes[j].get_name() == name and len(blazes[j].get_flow_rates()) >= 3:
                ...
                dip_magnitudes.append(blazes[j].get_minimum_dip() - blazes[j].get_expected_flow())
                dip_times.append(blazes[j].get_max_dip_time())

                # Determine which factors, if any, have failed, and add them to a list if they are not already there
        ...
        # Calculate the time resolution and flow percentage
        resolution = statistics.median(resolutions)
        flow_percentage = median / expected * 100.0

        # Adjust subplots to allow for text on the side, and prepare text boxes
        plt.subplots_adjust(top=0.85, right=0.7)
        props = dict(facecolor='none', edgecolor='black', pad=4)

        # Print title, whether or not the instrument run has passed or failed, and print run info in text boxes
        plot_title = name + "\n\n"
        ...
        plt.text(1.015, 0.85, "Time resolution = {0:.2f} s\nMed flow = {1:.2f} mL/min\nFlow percent = {2:.1f}%".format(resolution, median, flow_percentage), transform=ax.transAxes, bbox=props, fontsize=11)
        ...
        # Print error message if nothing was graphed
        if k == 0:
            plt.text(1.015, 0.5, "Error:\nFlow never reached \nexpected flow rate", transform=ax.transAxes, fontsize=11, bbox=props)

        # Set axis titles and scales
        ...
        file_name = path + "/" + plot_name + ".png"
        fig.savefig(file_name)


# Method to read from a file and store its data
def read_data():
    times.append([])
    flow_rates.append([])
...
# Finds the run name and expected flow rate from the name of the file
def get_run(file_names, index):
    ...
    if len(textsplit) < 4:
        print("Warning: file naming convention not followed for file {0}. Skipping file...".format(file_name))
        return None, None
    ...
    # Should not be necessary, but if the runs don't include "LHS" and "RHS", it assigns expected flows of 9 and 3
    else:
        if index % 2 == 0:
            exp_flow = 9
        else:
...
for i in range(0, len(files)):
    ...
    run, exp_flow = get_run(file_names, i)

    if run is not None and exp_flow is not None:
...
# Uncomment the following code if you want the error status printed to console
# for blaze in blazes:
#     blaze.pass_print()
